...
weather$DW[weather$Tavg_ThisWeek > Tbase] <- weather$Tavg_ThisWeek - Tbase
# Add a column with a cooling variable DC, as in paper
weather$DC <- rep(0,times=nrow(weather))
weather$DC[weather$Tavg_ThisWeek < Tbase] <- Tbase - weather$Tavg_ThisWeek
...
WnvP_by_all <- lapply(split(train$WnvPresent, list(train$Date,train$Trap,train$Species), drop = T,sep="*"),su)
...
rf <- randomForest(train, labels, ntree=10000, importance=TRUE, nodesize = 1)
...
write.csv(submission, file = "Mosquito_random_forest_in_R_PastWeather.csv", row.names=FALSE)