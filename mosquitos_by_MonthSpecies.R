...
names(train_cast_virus) <- c("Date","Trap","WnvP_in_Erraticus","WnvP_in_Pipiens","WnvP_in_Pipiens.Restuans","WnvP_in_Restuans","WnvP_in_Salinarius","WnvP_in_Tarsalis","WnvP_in_Territans")
  ...
  # Add a column with WnvP for an unspecified species (average of other species)
...
WnvP_Mean <- apply(train_cast_virus_split,1,mean)
WnvP_Mean[WnvP_Mean>=0.5] <- 1
WnvP_Mean[WnvP_Mean<0.5] <- 0
train_cast_virus$WnvP_Unspecified <- WnvP_Mean

  # Add a column with WnvP from any Mosquito species --> OK!
...
# Set to 1 if P>0.5
train_cast_merged$Date <- month(ymd(train_cast_merged$Date))
