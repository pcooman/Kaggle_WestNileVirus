...
# Add weather last year
# Probs from MLE by week
# Include TotNumMosquitos as feature to predict Prob
# Drop time indices from features (year, month, data, day, week, etc)
...
data_dir <- "input/"
train_MLE <- read.csv(file.path(data_dir, "train_MLE4.csv"))
...
#        train$Prob_by_week[counter] <- 0                                               # adjust probability WNV was found (=0)
...
test$Week <- week(ymd(test$Date))

# select only years 2009,2011 and 2013 (for which we have weather data from the previous year)
train <- subset(train, subset = (train$Year != 2007))
weather_prev <- subset(weather_merged,select = c("Date","Tavg.x","Tavg.y"))
names(weather_prev) <- c("Date","Tavg_prev.x","Tavg_prev.y")
weather_prev$Year <- year(ymd(weather_prev$Date))
weather_prev$Year <- weather_prev$Year + 1
weather_prev$Month <- month(ymd(weather_prev$Date))
weather_prev$Day <- day(ymd(weather_prev$Date))
weather_prev$Date <- NULL
...
train <- merge(train,weather_prev, by=c("Year","Month","Day"))
test <- merge(test,weather_prev, by=c("Year","Month","Day"))
train$Date <- NULL
test$Date <- NULL
...
labels2 <- train$Prob_by_week
train$Prob_by_week <- NULL
...
rf2 <- randomForest(train2, labels2, ntree=1000, importance=TRUE, nodesize = 5)
...
write.csv(submission, file = "Mosquito_random_forest_in_R_Weather_last_year.csv", row.names=FALSE)