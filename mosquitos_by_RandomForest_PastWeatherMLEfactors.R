...
data_dir <- "input/"
train_MLE <- read.csv(file.path(data_dir, "train_MLE3.csv"))
train_full <- read.csv(file.path(data_dir, "train_full.csv"))
...
#train <- subset(train_MLE,subset = (train_MLE$Species == "CULEX PIPIENS/RESTUANS" | train_MLE$Species == "CULEX PIPIENS" | train_MLE$Species == "CULEX RESTUANS"))
...
#        train$TotalNumMosquitos[counter] <- 0                                  # adjust the number of mosquitos found (=0)
#        train$Prob[counter] <- 0                                               # adjust probability WNV was found (=0)
#        train$WnvPresent[counter] <- 0                                         # adjust probability WNV was found (=0)
#      }
#    }
#  }
#}
#train_full <- arrange(train, Date,Trap,Species)
#write_csv(train_full,file.path(data_dir, "train_full.csv"))
...
# labels1_df <- split(train$TotalNumMosquitos,list(train$Date,train$Trap,train$Species))

labels1 <- train$TotalNumMosquitos
labels2 <- train$WnvPresent
...
train$Station.x <- NULL
train$Station.y <- NULL
train$Heat.x <- as.integer(train$Heat.x)
train$Cool.x <- as.integer(train$Cool.x)
train$Heat.y <- as.integer(train$Heat.y)
train$Cool.y <- as.integer(train$Cool.y)
train$Longitude_int <- NULL
train$AvgSpeed.y <- as.numeric(train$AvgSpeed.y)
train$D <- train$DW.x - train$DC.x
train$Sunrise.x <- as.integer(train$Sunrise.x)
train$Sunset.x <- as.integer(train$Sunset.x)
train$DW.x <- NULL
train$DW.y <- NULL
train$DC.x <- NULL
train$DC.y <- NULL
...
test$Station.x <- NULL
test$Station.y <- NULL
test$Heat.x <- as.integer(test$Heat.x)
test$Cool.x <- as.integer(test$Cool.x)
test$Heat.y <- as.integer(test$Heat.y)
test$Cool.y <- as.integer(test$Cool.y)
test$Longitude_int <- NULL
test$AvgSpeed.y <- as.numeric(test$AvgSpeed.y)
test$D <- test$DW.x - test$DC.x
test$Sunrise.x <- as.integer(test$Sunrise.x)
test$Sunset.x <- as.integer(test$Sunset.x)
test$DW.x <- NULL
test$DW.y <- NULL
test$DC.x <- NULL
test$DC.y <- NULL
...
train$TotalNumMosquitos <- labels1
test$TotalNumMosquitos <- TotalNumMosquitos_pred
...
# Predict Random forest
WnvP_pred <- predict(rf2, test)
WnvP_pred[WnvP_pred > 1] <-1
WnvP_pred[WnvP_pred < 0] <-0

# Set all other species to zero
# WnvP_pred[test$Species == 8] <- 0

submission <- data.frame(Id = 1:nrow(test), WnvPresent = WnvP_pred)
...
for (i in seq(from = 8, to = nrow(submission), by = 8)) {
  submission$WnvPresent[(i-4):(i-1)] <- 0 # mean(submission$WnvPresent[(i-7):(i-1)])
}

write.csv(submission, file = "Mosquito_random_forest_in_R_PastWeatherMLE_factor.csv", row.names=FALSE)
# 0.74893
# factors: 0.74526
# leaving unspecied (not set to zero): 0.54592