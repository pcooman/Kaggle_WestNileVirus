...
data_dir <- "input/"
train_MLE <- read.csv(file.path(data_dir, "train_MLE2.csv"))
train <- read.csv(file.path(data_dir, "train_full2.csv"))
...
# Split train --> tran and test
CV_logical <- c(rep("TRUE",times = nrow(train)*0.7), rep("FALSE",times = nrow(train)*0.3))
CV_logical <- sample(CV_logical)
CV_logical <- as.logical(CV_logical)

test <- train[!CV_logical,]
train <- train[CV_logical,]

truth3 <- test$WnvPresent
...
#write_csv(train_full,file.path(data_dir, "train_full2.csv"))
...
# Drop Longitude_int column (only one unique value)
...
labels2 <- train$Prob
truth1 <- test$TotalNumMosquitos
truth2 <- test$Prob

train$TotalNumMosquitos <- NULL
train$Prob <- NULL
test$TotalNumMosquitos <- NULL
test$Prob <- NULL
train$Year.x <- NULL
train$Year.y <- NULL
test$Year.x <- NULL
...
#  Prob_pred[(i-4):(i-1)] <- 0 # mean(submission$WnvPresent[(i-7):(i-1)])
#}

submission <- data.frame(Id = 1:nrow(test), WnvPresent = 1 - (1-Prob_pred)^TotalNumMosquitos_pred)

score1 <- sqrt(sum((TotalNumMosquitos_pred-truth1)^2))
score2 <- sqrt(sum((Prob_pred-truth2)^2))
score3 <- sqrt(sum((submission$WnvPresent-truth3)^2))

# write.csv(submission, file = "Mosquito_random_forest_in_R_PastWeatherMLE2.csv", row.names=FALSE)